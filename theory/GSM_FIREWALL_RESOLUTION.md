...
**Stabilizer generators from Coxeter reflections.**
...
These generators have a natural bipartition from the Coxeter-Dynkin
diagram H₄ = ●—●—●—●(5):

- **Even generators:** {s₁, s₃} (non-adjacent nodes — these commute)
- **Odd generators:** {s₂, s₄} (non-adjacent nodes — these commute)

The groups ⟨s₁, s₃⟩ and ⟨s₂, s₄⟩ each independently stabilize
subspaces of the 120-vertex Hilbert space. This bipartition is
suggestive of CSS structure, but the full CSS condition (C_X ⊆ C_Z^⊥)
requires verifying containment of the corresponding classical codes,
which we do not claim here. What matters for the firewall resolution
is the code parameters [[120, 6, 5]] — the monogamy escape works for
**any** stabilizer code with these parameters, regardless of CSS
structure. The code distance d = 5 guarantees that erasing any 2
physical qubits (vertices) leaves the 6 logical qubits intact.
...
3. **Quantum error-correcting code:** The 12-regular 600-cell graph implements a
   [[120, 6, 5]] stabilizer code (§5.3). The interior state is encoded in the
