...
\subsection{The KK--Casimir bridge}\label{sec:kkcasimir}

The gap between irrational Kaluza--Klein exponents (elements of
$\mathbb{Q}(\varphi)$) and integer Casimir exponents is closed by the
Galois structure of the field extension
$\mathbb{Q}(\varphi)/\mathbb{Q}$.  The 240 parallel fractions of $E_8$
roots are $\mathbb{Q}(\varphi)$ elements whose Galois conjugate pairs
sum to~2.  At one loop, Galois cancellation enforces
\emph{democracy}---all conjugate pairs contribute equally, producing
sector-independent corrections.  At two loops, cross-orbit mixing
between distinct Galois orbits introduces integer exponents via Casimir
labeling: the mixing amplitude at Casimir degree~$d$ contributes
$\varphi^{-d}$.  At three loops, doubled insertions of cross-orbit
vertices produce $\varphi^{-2d}$, generating the doubled Coxeter
pattern ($7 \to 14$, $8 \to 16$, $13 \to 26$).  All 24 GSM exponents
are produced; the Galois orbit structure quantizes the irrational KK
masses to integer Casimir exponents, closing the gap between the
algebraic (Molien) and dynamical (one-loop) frameworks.  See
\texttt{proofs/kk\_casimir\_bridge.py} in~\cite{GSMrepo}.
...
The total echo count $N_{\rm total} = 40$ is derived from the
half-hierarchy argument: the full exponent is~80, each echo spans 2
half-levels, giving $N = 80/2 = 40 = h + \text{rank} + c_1$.
Observable echoes $N_{\rm obs} \approx 7$--$12$ for current and
next-generation detectors.  Newton's constant~$G$ entering these
predictions is itself derived: the hierarchy formula
$\Mpl = v\,\varphi^{80-\varepsilon-\delta}$ IS the derivation of~$G$,
matching CODATA to $0.0001\%$.
...
The major theoretical gaps have been closed (see below).  Two
well-defined problems and six experimental tests remain:
...
any formula), a rigorous proof that this boundary aligns with Casimir
degree~$d_6$ is still open.
...
not been systematically analyzed.  These are acknowledged but not
urgent.

\item \emph{The six predictions} (Table~\ref{tab:predictions}) await
experimental confirmation or falsification.
\end{enumerate}

\noindent\textbf{Previously open gaps, now closed:}
\begin{itemize}
\item \emph{KK--Casimir bridge} (Sec.~\ref{sec:kkcasimir}): Galois
  structure of $\mathbb{Q}(\varphi)/\mathbb{Q}$ quantizes irrational KK
  masses to integer Casimir exponents.  All 24/24 exponents generated.
\item \emph{Newton's $G$}: The hierarchy formula
  $\Mpl = v\,\varphi^{80-\varepsilon-\delta}$ IS the derivation of~$G$.
  $G$ matches CODATA to $0.0001\%$.
\item \emph{$\Omega_\Lambda$}: Derived from
  $\varphi^{-1}+\varphi^{-2}=1$ (golden ratio geometric partition).
  Dark energy fraction is the $H_4$ projection eigenvalue.
\item \emph{BH entropy}: Wald entropy per hinge gives
  $S = A/(4\ell_P^2)$ exactly, fixing the previous $O(1)$ prefactor
  mismatch.
\item \emph{GW echoes}: $N_{\rm total}=40$ from half-hierarchy
  ($80/2$); $N_{\rm obs}\approx 7$--$12$ for current/future detectors.
\end{itemize}
