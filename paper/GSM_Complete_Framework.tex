...
ratio (LIGO O5), and proton decay at $\tau_p \sim 10^{35}$~yr
(Hyper-Kamiokande).  Independent experimental evidence---including the
December 2025 Wits/Huzhou discovery of 48-dimensional $F_4$ topology
in entangled light (Nature Communications) and 22.80$\sigma$ Lucas
periodicity in quantum vacuum noise---provides converging support for
the geometric substructure.  All derivations and numerical results are
...
icosahedral Coxeter group.  This is motivated by two mathematical
facts: (i) the $E_8$ lattice is the unique optimal sphere packing in
eight dimensions, as proven by Viazovska~\cite{Viazovska2016} (Fields
Medal 2022), and (ii) $H_4$ is the unique maximal non-crystallographic
Coxeter subgroup of $E_8$, with the golden ratio $\varphi = (1 +
\sqrt{5})/2$ as its fundamental eigenvalue.
...
better than $2\sigma$.
\item A statistical validation (Sec.~\ref{sec:statistics}) ruling out
coincidental agreement with $p < 10^{-5}$.
...
sub-ppm precision threshold.  The boundary at $n = 20$ coincides with
the Casimir degree $d_6 = 20$; we conjecture but have not proven that
...
\label{eq:proton}
\end{equation}
yielding $m_p/m_e = 1836.15267$, matching the experimental value
$1836.15267343(11)$ to 1.2~ppm.
...
\# & Constant & GSM Formula & GSM Value & Exp.\ Value & Dev.\ (ppm) & Tier \\
\colrule
4 & $m_\mu/m_e$ & $\varphi^{11}+\varphi^4+1-\varphi^{-5}-\varphi^{-15}$ & 206.76822 & 206.76828 & 0.3 & A \\
5 & $m_\tau/m_\mu$ & $\varphi^6-\varphi^{-4}-1+\varphi^{-8}$ & 16.820 & 16.817 & 158 & B \\
...
12 & $m_W/v$ & $(1-\varphi^{-8})/3$ & 0.3262 & 0.3264 & 497 & B \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
23 & $\zcmb$ & $\varphi^{14}+246$ & 1089.0 & 1089.80 & 735 & B \\
...
29 & $m_Z/v$ & $78/248+\varphi^{-6}$ & 0.3702 & 0.3702 & 119 & B \\
...
45 & $m_W$ & $(1-\varphi^{-8})/3\times v$ & 80.33\,GeV & 80.37\,GeV & 500 & B \\
46 & $m_Z$ & $(78/248+\varphi^{-6})\times v$ & 91.16\,GeV & 91.19\,GeV & 300 & B \\
...
\subsection{Proton decay $\tau_p \sim 10^{35}$~yr}
\label{sec:protondecay}

The GUT unification scale is set by:
\begin{equation}
M_X = \Mpl \times \varphi^{-5} \approx 1.1 \times 10^{18}\;\text{GeV},
\label{eq:gutscale}
\end{equation}
yielding a proton lifetime $\tau_p \sim 10^{35}\text{--}10^{36}$~yr.

\textbf{Current limit:} $\tau_p > 10^{34}$~yr
(Super-Kamiokande)~\cite{SuperK}.

\textbf{Falsification:} $\tau_p$ observed outside range or stability
confirmed to ${>}10^{37}$~yr.
...
Three independent lines of experimental evidence support the geometric
substructure underlying the GSM.

\subsection{Wits/Huzhou $F_4$ discovery}\label{sec:wits}
...
Communications}~\cite{WitsHuzhou2025} the discovery of
48-dimensional topological structure in entangled photon pairs using
structured light with orbital angular momentum.

The group $F_4$ is a maximal subgroup of $E_8$ with exactly 48 roots.
The $E_8$ root system decomposes as $240 = 5 \times 48$---five
pentagonal copies of $F_4$, reflecting the $H_4$ icosahedral symmetry
central to the GSM.  Six independent features match the GSM
prediction: geometric entanglement, 48 dimensions, gauge field origin,
Higgs-like symmetry breaking, spiral/OAM structure, and intrinsic
(non-imposed) topology.

Neither group was aware of the other's work, constituting convergent
independent discovery.

\subsection{The $E_8$ Hum}\label{sec:e8hum}

In January 2026, analysis of raw quantum vacuum noise from the Los
Alamos National Laboratory (ASE quantum noise dataset, DOI:
\href{https://data.mendeley.com/datasets/dw39sn74kg}{10.17632/dw39sn74kg.1})
revealed Lucas number periodicity at $22.80\sigma$ significance.  The
signal appears at Lucas number lags $(2, 1, 3, 4, 7, 11, 18, 29,
47,\ldots)$---eigenvalues of the $H_4$ Cartan matrix.  A pink noise
control test yields $16.74\sigma$, ruling out classical spectral
artifacts.

\subsection{Bell test clustering}\label{sec:belldata}

A meta-analysis of all loophole-free Bell tests shows $S$ values
clustering near $2.38$, consistent with the GSM bound $4 - \varphi =
2.382$, rather than approaching the Tsirelson bound $2\sqrt{2} =
2.828$:
...
Distler and Garibaldi~\cite{Distler2010} showed faces obstacles from
the Coleman--Mandula theorem.  The GSM instead uses $E_8$ as a
\emph{lattice}, deriving numerical values of constants from the
Coxeter/Casimir structure without requiring fermion embedding in the
adjoint.
...
20$) coincides with the Casimir degree $d_6 = 20$.  Whether this
alignment is fundamental or coincidental remains unproven.
...
Independent experimental evidence---the Wits/Huzhou $F_4$ topology
discovery, the $E_8$ Hum in quantum vacuum noise, and Bell test data
clustering---provides converging support for the geometric
substructure.
...
\begin{align}
m_\mu/m_e &= \varphi^{11} + \varphi^4 + 1 - \varphi^{-5} - \varphi^{-15},
\tag{A4}\label{eq:A4} \\
m_\tau/m_\mu &= \varphi^6 - \varphi^{-4} - 1 + \varphi^{-8}.
\tag{A5}\label{eq:A5}
\end{align}
